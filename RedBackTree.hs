...
instance NFData a => NFData (Tree a) where
  rnf E = ()
  rnf (T color left value right) = color `deepseq` rnf left `deepseq` rnf right
...
-- We have 2 options
-- 1. If it(the current node i.e t) is black, delete from t1 and then balance.
...
-- We have 2 options
-- 1. If it(the current node i.e t) is black, delete from t2 and then balance.
...
fuse (T B t1 x t2) (T B t3 y t4)  =
  let s = fuse t2 t3
  in case s of
       (T R s1 z s2) -> (T R (T B t1 x s1) z (T B s2 y t4)) -- confusing case

-- Helper function to print the tree
showTree :: Show a => Tree a -> String
showTree E = "E"
showTree (T R l x r) = "(R " ++ showTree l ++ " " ++ show x ++ " " ++ showTree r ++ ")"
showTree (T B l x r) = "(B " ++ showTree l ++ " " ++ show x ++ " " ++ showTree r ++ ")"
...
main :: IO ()
main = do
  -- Explicit Type Annotations for small and large trees
  let smallTree :: Tree Int
      smallTree = insert 5 (insert 3 (insert 7 (insert 2 (insert 4 empty))))

      largeTree :: Tree Int
      largeTree = foldr insert empty [1..100000]
  ...
  defaultMain
    [ bgroup "RedBlackTree"
        [ bgroup "insert"
            [ bench "smallTree" $ nf (insert 6) smallTree
            , bench "largeTree" $ nf (insert 100001) largeTree
            ]
        , bgroup "member"
            [ bench "smallTree" $ nf (member 4) smallTree
            , bench "largeTree" $ nf (member 50000) largeTree
            ]
        ]
    ]
